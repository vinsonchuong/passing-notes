...
test('handling only upgrades to WebSocket', async t => {
  const webSocketServer = new WebSocket.Server({ noServer: true })
  webSocketServer.on('connection', webSocket => {
    webSocket.on('message', message => {
      t.is(message, 'Ping')
  ...
  const server = await startServer(10014, null, (request, socket, head) => {
    webSocketServer.handleUpgrade(request, socket, head, webSocket => {
      webSocketServer.emit('connection', webSocket, request)
    })
  })

  const clientSocket = await fromWebSocket('ws://localhost:10014')
  clientSocket.send('Ping')
  clientSocket.close()
  t.is((await clientSocket.next()).value, 'Pong')
