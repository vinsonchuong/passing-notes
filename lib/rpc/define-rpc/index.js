...
export default function<
  ...
  Dependencies: { log: (Log, ?Log) => void }
>(
  actions: Actions
): {
  serveRpc: Dependencies => Responder => Responder,
  sendRpc: <
    Action: $Keys<Actions>,
    Params: { [string]: Json },
    Result: $Call<$Call<$ElementType<Actions, Action>, Dependencies>, Params>
  >({ action: Action, params: Params }) => Promise<Result>,
  api: $ObjMap<Actions, <T>((Dependencies) => T) => T>
} {
  function serveRpc(dependencies) {
    ...
    return next => async request => {
      ...
      try {
        const result = await actions[parsedBody.action](dependencies)(
          parsedBody.params
        )
