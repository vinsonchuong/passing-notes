...
test.serial('creating a client and server middleware', async t => {
  const { api, sendRpc, serveRpc } = defineRpc({
    ...
    subtract: () => ({ a, b }: { a: number, b: number }): number => a - b,
    constant: ({
      getMagicNumber
    }: {
      getMagicNumber: () => number
    }) => (): number => getMagicNumber()
  })
  ...
  t.deepEqual(await sendRpc({ action: 'subtract', params: { a: 1, b: 2 } }), {
    result: -1
  })
  t.is(await api.subtract({ a: 1, b: 2 }), -1)
