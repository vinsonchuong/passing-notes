/* @flow */
import type { Responder } from 'passing-notes/lib/http'
import { promisify } from 'util'
import * as path from 'path'
import * as fs from 'fs'
import * as url from 'url'
import waitForEvent from 'promisify-event'
import Bundler from 'parcel-bundler'
import JsAsset from 'passing-notes/lib/bundler/parcel/JSAsset'

const dist = path.resolve('dist')
const readFile = promisify(fs.readFile)

function getParcelConfig() {
  if (process.env.NODE_ENV === 'test') {
    return {
      outDir: dist,
      publicUrl: '/',
      logLevel: 0,
      cache: false
    }
  } else {
    return {
      outDir: dist,
      publicUrl: '/'
    }
  }
}

export default function(entryPoint: string): Responder => Responder {
  const bundler = getBundler(entryPoint)

  return next => {
    return async request => {
      if (bundler.pending) {
        await waitForEvent(bundler, 'bundled')
      }

      const { pathname } = url.parse(request.url)

      if (pathname === '/') {
        return {
          status: 200,
          headers: {},
          body: await readFile(path.join(dist, 'index.html'), 'utf8')
        }
      } else if (typeof pathname === 'string') {
        return {
          status: 200,
          headers: {},
          body: await readFile(path.join(dist, pathname), 'utf8')
        }
      } else {
        return next(request)
      }
    }
  }
}

let currentBundler, currentEntryPoint
function getBundler(entryPoint) {
  if (currentBundler && entryPoint !== currentEntryPoint) {
    currentBundler.stop()
    currentBundler = null
  }

  if (!currentBundler) {
    currentEntryPoint = entryPoint
    currentBundler = new Bundler(entryPoint, getParcelConfig())
    currentBundler.addAssetType('js', JsAsset.path)
    currentBundler.bundle()
  }

  return currentBundler
}
