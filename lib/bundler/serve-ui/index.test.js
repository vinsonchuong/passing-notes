...
testWithBrowser.serial('serving a UI', async t => {
...
test.serial('setting Cache-Control headers to no-cache in development', async t => {
...
test.serial('setting Cache-Control headers in production', async t => {
...
test.serial('serving index.html for any unknown paths', async t => {
  const respond = serveUi({ entry: htmlFixturePath, log: () => () => {} })(
